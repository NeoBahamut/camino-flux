...
const fileNameFormat = ({
  ...
  return `titres${
    domainesIds ? '-' + domainesIds.map((d) => d).join(',') : ''
  }${typesIds ? '-' + typesIds.map((t) => t).join(',') : ''}${
    statutsIds ? '-' + statutsIds.map((s) => s).join(',') : ''
  }${
    noms
      ? '-' +
        noms
...
const titresGet = async (req, res) => {
  try {
    ...
    // track Matomo
    matomo.track({
      url: `${fileNameFormat(variables)}`,
      action_name: "Import dynamique d'un flux Geojson",
      download: `http://${fileNameFormat(variables)}`,
    })
